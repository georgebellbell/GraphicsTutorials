...
void main() {
  ...
  vec4 positionFrom = texture(position, texCoord);
  ...
  for (i = 0; i < int(delta); ++i) {
    frag      += increment;
    uv.xy      = frag / texSize;
    positionTo = texture(position, uv.xy);
    ...
    depth        = viewDistance - positionTo.y; // perspective correct interpolation
  ...
  steps *= hit0; // if the ray didnt hit anything on its first pass, skip the second one
